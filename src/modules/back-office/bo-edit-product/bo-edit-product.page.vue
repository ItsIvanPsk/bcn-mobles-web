    ...
    <div class="max-w-7xl mx-auto grid lg:grid-cols-3 gap-12">
      ...
      <div class="lg:col-span-2 bg-white rounded-xl shadow-lg p-8 space-y-12">
        ...
        <section>
          ...
          <input v-model="product.name" class="w-full border rounded px-3 py-2" />
        </section>

        <!-- Descripción corta -->
        <section>
          <h2 class="text-xl font-semibold mb-2">Descripción corta</h2>
          <input v-model="product.shortDescription" class="w-full border rounded px-3 py-2" />
        </section>

        <!-- Descripción -->
        <section>
          <h2 class="text-xl font-semibold mb-2">Descripción</h2>
          <textarea v-model="product.description" class="w-full border rounded px-3 py-2"></textarea>
        </section>

        <!-- Precio -->
        <section>
          <h2 class="text-xl font-semibold mb-2">Precio (€)</h2>
          <input type="number" v-model="product.price" class="w-full border rounded px-3 py-2" />
        </section>
      ...
      <div class="flex flex-col gap-10">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6">
          ...
          <div class="flex gap-2">
            <select v-model="newCategoryId" class="border rounded px-3 py-2 flex-1">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6">
          ...
          <div class="flex flex-wrap gap-2 mt-3">
            ...
            >
              {{ s }}
              <button @click="removeSize(i)" class="text-red-500 text-xs">✕</button>
            </span>
          </div>
        </div>
      </div>
      ...
      <div class="flex flex-col gap-10">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6">
          ...
          <div class="flex gap-2">
            <input
              ...
              class="border rounded px-3 py-2 flex-1"
            />
            <input type="color" v-model="newColorHex" class="w-12 h-10 border rounded" />
            <button
              @click="addColor"
              class="bg-gray-200 px-3 py-1 rounded text-sm"
            >
          ...
          <div class="flex flex-wrap gap-2 mt-3">
            <div
              ...
              :key="i"
              class="flex items-center gap-2 px-2 py-1 border rounded"
            >
        ...
        <div class="bg-white rounded-xl shadow-lg p-6">
          ...
          <input
            ...
            class="w-full border rounded px-3 py-2 mb-2"
          />
          <input type="file" @change="onMainImageUpload" />
          <img
        ...
        <div class="bg-white rounded-xl shadow-lg p-6">
          ...
          <div class="space-y-2">
            ...
            >
              <input v-model="product.images[i].src" class="flex-1 border rounded px-3 py-2" placeholder="URL" />
              <input v-model="product.images[i].alt" class="flex-1 border rounded px-3 py-2" placeholder="Texto alternativo" />
              <button @click="removeImage(i)" class="text-red-600 hover:underline text-sm">
                Eliminar
              </button>
            </div>
          </div>
          <div class="mt-3 flex gap-2">
            <input type="file" @change="onGalleryUpload" />
            <button @click="addImage" class="bg-gray-200 px-3 py-1 rounded text-sm">
              + Añadir por URL
            </button>
          </div>
        </div>
...
function addSize() {
  ...
  if (size && !product.value.sizes.includes(size)) {
    product.value.sizes.push(size)
    newSize.value = ""
  }
}
function removeSize(i: number) {
  product.value.sizes.splice(i, 1)
}
